...
const previousPrices = await stateStore.getAutoSavedValue<Record<string, number>>(
    PREVIOUS_PRICES_KEY,
    {},
);
...
router.addHandler(Labels.PRODUCT, async ({ log, request, $, body }) => {
    ...
    const products = combinationInfo.map((combination) => {
        ...
        // Get previous price, handle change of current price
        const previousPrice = previousPrices[uniqueProductKey];
        const priceChange = previousPrice ? listPrice - previousPrice : null;
        previousPrices[uniqueProductKey] = listPrice;
